...
import type { Config, SurveyAnswers } from "../types";
...
export default function AutoResult() {
  ...
  useEffect(() => {
    if (!answers) return;
    const t = window.setTimeout(() => setLoadState("done"), 400);
  ...
  const cfg: Config = buildRecommendation(answers);
  ...
  const handleSave = () => {
    ...
    const res = saveConfigAction({ ...cfg, id: uid("cfg"), updatedAt: Date.now() });
    toast(res.message);
  };
